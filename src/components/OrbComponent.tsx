...
export const OrbComponent: React.FC<OrbProps> = ({
  ...
  // Add hover threshold constant after other refs
  const HOVER_THRESHOLD = 0.2; // Adjusts the size of the hover-sensitive area
  ...
  useFrame((state, delta) => {
    ...
    // Type-safe position access
    const positions = pointsRef.current.geometry.attributes.position
      .array as Float32Array;

    // Handle anger level transition
    if (hovered && !voiceMode) {
      setAngerLevel(Math.min(1, angerLevel + delta * angerSpeed));
    } else {
      setAngerLevel(Math.max(0, angerLevel - delta * angerSpeed));
    }
    ...
    if (formationProgress.current < 1) {
      ...
      for (let i = 0; i < grainCount; i++) {
        const idx = i * 3;
        const patternType = formationPatterns.current?.[i] ?? 0;
        ...
        const targetX = initialPositions[idx] ?? 0;
        const targetY = initialPositions[idx + 1] ?? 0;
        const targetZ = initialPositions[idx + 2] ?? 0;

        const startX = initialFormationPositions.current?.[idx] ?? 0;
        const startY = initialFormationPositions.current?.[idx + 1] ?? 0;
        const startZ = initialFormationPositions.current?.[idx + 2] ?? 0;
    ...
    if (materialRef.current) {
      ...
      // Enhance glow effect when angry
      materialRef.current.size = grainSize * (1 + angerLevel * 1.5);
    }
    ...
    if (formationProgress.current >= 1) {
      ...
      if (!hovered || voiceMode) {
        for (let i = 0; i < grainCount; i++) {
          const i3 = i * 3;
          const initialX = initialPositions[i3] ?? 0;
          const initialY = initialPositions[i3 + 1] ?? 0;
          const initialZ = initialPositions[i3 + 2] ?? 0;
          ...
          if (isExploding) {
            ...
            if (phase === 0) {
              ...
              const pos = new Vector3(
                initialX + (explosionVector?.x ?? 0),
                initialY + (explosionVector?.y ?? 0),
                initialZ + (explosionVector?.z ?? 0),
              );
        ...
        // Apply spiky angry effect when hovered
        for (let i = 0; i < grainCount; i++) {
          const i3 = i * 3;
          const initialX = initialPositions[i3] ?? 0;
          const initialY = initialPositions[i3 + 1] ?? 0;
          ...
          // Create spiky distortion
          // const pos = new Vector3(initialX, initialY, initialZ).normalize();
          const spikeNoise = Math.sin(time.current * 5 + i * 0.1) * 0.5 + 0.5;
          const spikeFactor = spikeStrength * angerLevel * spikeNoise;

          const finalX = initialX * (1 + spikeFactor);
          const finalY = initialY * (1 + spikeFactor);
          const finalZ = initialZ * (1 + spikeFactor);

          positions[i3] = MathUtils.lerp(positions[i3] ?? 0, finalX, delta * 4);
          positions[i3 + 1] = MathUtils.lerp(
            positions[i3 + 1] ?? 0,
            finalY,
            delta * 4,
  ...
  const handlePointerMove = (e: ThreeEvent<PointerEvent>) => {
    ...
    // Only update hover state if within threshold
  ...
  const handleTap = (e: ThreeEvent<MouseEvent>) => {
    ...
    if (!isExploding) {
      ...
      // Reset particle positions with type safety
    ...
    <>
      <points
        ...
        onPointerOver={(e) => {
          if (voiceMode) return;
          e.stopPropagation();
          // Don't set hover state here - it's handled in handlePointerMove
        }}
        onPointerOut={(e) => {
          if (voiceMode) return;
          e.stopPropagation();
